...
object BuildConfig {
  ...
  object BuildInfoConfig {
    val serviceName         = BuildInfoKey.map(name)(_._1 -> "device-data-handler-service")
    val packageName         = "org.tomohavvk.walker"
    val buildInfoKeysCustom = Seq[BuildInfoKey](version, scalaVersion, sbtVersion, serviceName)
